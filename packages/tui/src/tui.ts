...
import { $flag, getDebugLogPath } from "@oh-my-applepie/pi-utils";
...
export class TUI extends Container {
	...
	#doRender(): void {
		...
		const preserveScrollback = this.terminal.preserveScrollbackOnFullRedraw;
		...
		const fullRender = (clear: boolean): void => {
			...
			if (clear) buffer += preserveScrollback ? "\x1b[2J\x1b[H" : "\x1b[2J\x1b[H\x1b[3J";
		...
		if (heightChanged && !isTermuxSession() && !preserveScrollback) {
