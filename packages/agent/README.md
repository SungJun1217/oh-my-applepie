# @oh-my-applepie/pi-agent

Stateful agent with tool execution and event streaming. Built on `@oh-my-applepie/pi-ai`.

## Installation

```bash
npm install @oh-my-applepie/pi-agent
```

## Quick Start

```typescript
import { Agent } from "@oh-my-applepie/pi-agent";
import { getModel } from "@oh-my-applepie/pi-ai";
...
```typescript
declare module "@oh-my-applepie/pi-agent" {
...
Define tools using `AgentTool` with a Zod parameter schema (via `z` from `@oh-my-applepie/pi-ai`).

```typescript
import { z } from "@oh-my-applepie/pi-ai";
...
```typescript
import { Agent, streamProxy } from "@oh-my-applepie/pi-agent";
...
```typescript
import { agentLoop, agentLoopContinue } from "@oh-my-applepie/pi-agent";
