...
- Removed the `@oh-my-applepie/pi-agent-core/compaction/handoff` exports from the package surface, including `extractHandoffDocument`, `createHandoffContext`, and `createHandoffFileName`
...
- Added `generateHandoff(messages, model, apiKey, options)` to `@oh-my-applepie/pi-agent-core/compaction` to generate a handoff document by calling the model directly, using live system/tool context and optional metadata
...
- Added compaction APIs under `@oh-my-applepie/pi-agent-core/compaction`, including context compaction, branch summarization, handoff prompt/context helpers, pruning, token budgeting, prompt templates, and OpenAI `/responses/compact` helpers.
...
- **Agent loop moved from `@oh-my-applepie/pi-ai`**: The `agentLoop`, `agentLoopContinue`, and related types have moved to this package. Import from `@oh-my-applepie/pi-agent` instead.
