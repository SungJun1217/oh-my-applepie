...
Multi-agent orchestration for oh-my-applepie. Define agent workflows in YAML — pipelines, parallel fan-outs, sequential chains, or any DAG — and run them unattended until completion.

Each agent is a full oh-my-applepie subagent with access to every tool: bash, python, read, write, edit, grep, find, fetch, web_search, browser. The orchestrator manages lifecycle and ordering; agents communicate through the shared workspace filesystem.
...
Each agent is a full oh-my-applepie session. It can:
...
src/swarm/
  ...
  executor.ts         Spawns agents via oh-my-applepie's runSubprocess
