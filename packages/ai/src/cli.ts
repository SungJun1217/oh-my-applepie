...
async function main(): Promise<void> {
	...
	if (!command || command === "help" || command === "--help" || command === "-h") {
		console.log(`Usage: bunx @oh-my-applepie/pi-ai <command> [provider]
...
Examples:
  ...
  bunx @oh-my-applepie/pi-ai logout anthropic   # logout from specific provider
  bunx @oh-my-applepie/pi-ai status             # show logged-in providers
  bunx @oh-my-applepie/pi-ai list               # list providers
`);
		return;
	}
	...
	if (command === "status") {
		...
		try {
			...
			if (providers.length === 0) {
				...
				console.log(`Use 'bunx @oh-my-applepie/pi-ai login' to authenticate.`);
			} else {
	...
	if (command === "login") {
		...
		if (!PROVIDERS.some(p => p.id === provider)) {
			...
			console.error(`Use 'bunx @oh-my-applepie/pi-ai list' to see available providers`);
			process.exit(1);
		}
	...
	console.error(`Use 'bunx @oh-my-applepie/pi-ai --help' for usage`);
	process.exit(1);
}
