#!/usr/bin/env bun
import { installH2Fetch } from "@oh-my-applepie/pi-ai";
import { APP_NAME, MIN_BUN_VERSION, procmgr, VERSION } from "@oh-my-applepie/pi-utils";
...
// `HTTP2Unsupported`. See @oh-my-applepie/pi-ai/utils/h2-fetch for details.
installH2Fetch();
 ...
 */
import { type CliConfig, type CommandEntry, run } from "@oh-my-applepie/pi-utils/cli";
...
async function showHelp(config: CliConfig): Promise<void> {
	const { renderRootHelp } = await import("@oh-my-applepie/pi-utils/cli");
...
async function runSmokeTest(): Promise<void> {
	const { smokeTestSyncWorker } = await import("@oh-my-applepie/omp-stats");
