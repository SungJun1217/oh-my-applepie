...
} from "@oh-my-applepie/pi-ai/providers/openai-codex-responses";
...
export async function createAgentSession(options: CreateAgentSessionOptions = {}): Promise<CreateAgentSessionResult> {
	...
	// Kick off extension discovery early to overlap with model resolution.
	const extensionsPromise: Promise<LoadExtensionsResult> | undefined =
		options.disableExtensionDiscovery || options.preloadedExtensions
			? undefined
			: logger.time("discoverAndLoadExtensionsEarly", () => {
					const configuredPaths = [
						...(options.additionalExtensionPaths ?? []),
						...(settings.get("extensions") ?? []),
					];
					return discoverAndLoadExtensions(configuredPaths, cwd, eventBus, disabledExtensionIds);
				});
	...
	try {
		...
		} else if (extensionsPromise) {
			extensionsResult = await extensionsPromise;
			for (const { path, error } of extensionsResult.errors) {
				logger.error("Failed to load extension", { path, error });
			}
		} else {
