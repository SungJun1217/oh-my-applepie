...
export function prepareRetentionTranscript(
	...
	// Cap full-session retains (~12.5K tokens) to prevent oversized uploads.
	if (retainFullWindow && transcript.length > MAX_RETAIN_CHARS) {
		const originalLength = transcript.length;
		transcript = `${transcript.slice(0, MAX_RETAIN_CHARS)}\n\n… [transcript truncated at ${MAX_RETAIN_CHARS.toLocaleString()} chars — ${originalLength.toLocaleString()} total]`;
	}
