...
import { APP_NAME, formatDuration, formatNumber, formatPercent } from "@oh-my-applepie/pi-utils";
...
async function printStatsSummary(): Promise<void> {
	const { getDashboardStats } = await import("@oh-my-applepie/omp-stats");
	const stats = await getDashboardStats();
