...
import { DEFAULT_PRUNE_CONFIG, pruneToolOutputs } from "@oh-my-applepie/pi-agent-core/compaction/pruning";
import type {
	AssistantMessage,
	Context,
	Effort,
...
import { MacOSPowerAssertion } from "@oh-my-applepie/pi-natives";
import {
	extractRetryHint,
	getAgentDbPath,
	isEnoent,
...
export class AgentSession {
	...
	async #compactWithFallbackModel(
		...
		// Filter candidates with valid credentials before racing.
		// getApiKey may have side effects (credential refresh) so we run these serially.
		const authenticated: Array<{ model: Model; apiKey: string }> = [];
		for (const candidate of candidates) {
			const apiKey = await this.#modelRegistry.getApiKey(candidate, this.sessionId);
			if (apiKey) authenticated.push({ model: candidate, apiKey });
		}
		...
		if (authenticated.length === 1) {
			const { model, apiKey } = authenticated[0];
			try {
				return await compact(preparation, model, apiKey, customInstructions, signal, {
					...options,
					metadata: this.agent.metadataForProvider(model.provider),
					convertToLlm,
					telemetry,
				...
				if (!this.#isCompactionAuthFailure(error)) throw error;
				throw this.#buildCompactionAuthError();
			}
		}

		// Race candidates: first to succeed wins. Losers are aborted eagerly to
		// minimize wasted LLM tokens from in-flight streaming responses.
		const raceController = new AbortController();
		const raceSignal = AbortSignal.any([signal, raceController.signal]);

		try {
			const result = await Promise.any(
				authenticated.map(({ model, apiKey }) =>
					compact(preparation, model, apiKey, customInstructions, raceSignal, {
						...options,
						metadata: this.agent.metadataForProvider(model.provider),
						convertToLlm,
						telemetry,
			...
			if (error instanceof AggregateError) {
				const nonAuthErrors = error.errors.filter(e => !this.#isCompactionAuthFailure(e));
				if (nonAuthErrors.length > 0) throw nonAuthErrors[0];
			}
			throw this.#buildCompactionAuthError();
		} finally {
	...
	async #runAutoCompaction(
		...
		try {
			...
			if (compactionPrep.kind === "fromHook") {
				...
				if (authenticated.length === 0) {
					throw new Error("Compaction failed: no authenticated model available");
				}

				// Race per-candidate retry loops. Each candidate runs its own retry
				// loop in parallel; the first to produce a result wins. Losers are
				// aborted eagerly to minimize wasted tokens.
				const raceController = new AbortController();
				const raceSignal = AbortSignal.any([autoCompactionSignal, raceController.signal]);

				const candidateRace = (model: Model, apiKey: string): Promise<CompactionResult> => {
					return (async (): Promise<CompactionResult> => {
						let attempt = 0;
						while (true) {
							throwIfAborted(raceSignal);
							try {
								return await compact(preparation, model, apiKey, undefined, raceSignal, {
									promptOverride: compactionPrep.hookPrompt,
									extraContext: compactionPrep.hookContext,
									remoteInstructions: this.#baseSystemPrompt.join("\n\n"),
									metadata: this.agent.metadataForProvider(model.provider),
								...
								if (delayMs > maxAcceptableDelayMs) {
									logger.warn("Auto-compaction retry delay too long, dropping candidate", {
										delayMs,
										retryAfterMs,
										error: message,
								...
								await scheduler.wait(delayMs, { signal: raceSignal });
							}
						}
					})();
				};
				...
				try {
					compactResult = await Promise.any(
						authenticated.map(({ model, apiKey }) => candidateRace(model, apiKey)),
					);
				} catch (error) {
					if (error instanceof AggregateError) {
						const nonAuth = error.errors.find(e => !this.#isCompactionAuthFailure(e));
						throw nonAuth ?? error.errors[0] ?? new Error("Compaction failed: no available model");
					}
					throw error;
				} finally {
					raceController.abort();
