...
} from "@oh-my-applepie/pi-coding-agent/tools/gh";
import * as git from "@oh-my-applepie/pi-coding-agent/utils/git";
import { getAgentDir, hashPath, setAgentDir } from "@oh-my-applepie/pi-utils";
...
async function setupTempHome(): Promise<{ home: string; cleanup: () => Promise<void> }> {
	...
	setAgentDir(path.join(home, ".omap", "agent"));
	return {
		home,
		cleanup: async () => {
...
async function expectedWorktreePath(home: string, primaryRoot: string, localBranch: string): Promise<string> {
	...
	return fs.realpath(path.join(home, ".omap", "wt", segment));
}
