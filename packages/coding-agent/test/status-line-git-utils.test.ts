...
import {
	...
	parseDefaultBranch,
	parseGitHubRepo,
} from "@oh-my-applepie/pi-coding-agent/modes/components/status-line/git-utils";

describe("parseGitHubRepo", () => {
	test("parses HTTPS URL", () => {
		expect(parseGitHubRepo("https://github.com/can1357/oh-my-applepie.git")).toBe("can1357/oh-my-applepie");
	});

	test("parses HTTPS URL without .git suffix", () => {
		expect(parseGitHubRepo("https://github.com/can1357/oh-my-applepie")).toBe("can1357/oh-my-applepie");
	});

	test("parses SSH scp-style URL", () => {
		expect(parseGitHubRepo("git@github.com:loftiskg/oh-my-applepie.git")).toBe("loftiskg/oh-my-applepie");
	});

	test("parses SSH scp-style URL without .git suffix", () => {
		expect(parseGitHubRepo("git@github.com:loftiskg/oh-my-applepie")).toBe("loftiskg/oh-my-applepie");
	});
