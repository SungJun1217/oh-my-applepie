...
import { getConfigDirs } from "@oh-my-applepie/pi-coding-agent/config";
import { getUserPath } from "@oh-my-applepie/pi-coding-agent/discovery/helpers";
...
describe("PI_CONFIG_DIR", () => {
	...
	test("getConfigDirs respects PI_CONFIG_DIR for user base", () => {
		...
		expect(result[0]).toEqual({ path: expected, source: ".omap", level: "user" });
	});
});
