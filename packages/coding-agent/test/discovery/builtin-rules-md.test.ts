...
import { getCapability } from "@oh-my-applepie/pi-coding-agent/capability";
import { clearCache } from "@oh-my-applepie/pi-coding-agent/capability/fs";
import { type Rule, ruleCapability } from "@oh-my-applepie/pi-coding-agent/capability/rule";
import type { LoadContext } from "@oh-my-applepie/pi-coding-agent/capability/types";
// Register all discovery providers as a side effect.
import "@oh-my-applepie/pi-coding-agent/discovery";
...
test("user ~/.omp/agent/RULES.md becomes an alwaysApply rule", async () => {
	writeFile(
		path.join(home, ".omap", "agent", "RULES.md"),
...
test("project .omp/RULES.md becomes an alwaysApply rule", async () => {
	writeFile(path.join(project, ".omap", "RULES.md"), "# Project rule\nAlways say hi.\n");
...
test("project RULES.md is found walking up from a sub-package cwd", async () => {
	...
	writeFile(path.join(project, ".omap", "RULES.md"), "# Repo-wide sticky rule\n");
	...
	expect(projectRule).toBeDefined();
	expect(projectRule?.alwaysApply).toBe(true);
	expect(projectRule?.path).toBe(path.join(project, ".omap", "RULES.md"));
});

test("alwaysApply is forced even when frontmatter says false", async () => {
	writeFile(path.join(home, ".omap", "agent", "RULES.md"), "---\nalwaysApply: false\n---\nStick around anyway.\n");
...
test("absent RULES.md does not produce a rule", async () => {
	...
	writeFile(path.join(home, ".omap", "agent", "rules", "other.md"), "# Unrelated rule\n");
