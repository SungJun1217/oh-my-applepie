...
import { type ContextFile, contextFileCapability } from "@oh-my-applepie/pi-coding-agent/capability/context-file";
import { resetSettingsForTest, Settings } from "@oh-my-applepie/pi-coding-agent/config/settings";
import { initializeWithSettings, loadCapability } from "@oh-my-applepie/pi-coding-agent/discovery";
...
describe("disabledExtensions runtime filtering", () => {
	...
	beforeEach(async () => {
		...
		await fs.mkdir(path.join(tempDir, ".omap"), { recursive: true });
		await fs.writeFile(path.join(tempDir, ".omap", "AGENTS.md"), "# project instructions\n");
