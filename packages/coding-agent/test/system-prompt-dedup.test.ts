...
} from "@oh-my-applepie/pi-coding-agent/system-prompt";
...
describe("SYSTEM.md prompt assembly", () => {
	...
	it("renders SYSTEM.md exactly once when it is used as the custom base prompt", async () => {
		...
		const systemDir = path.join(projectDir, ".omap");
	...
	it("prefers project SYSTEM.md over user SYSTEM.md", async () => {
		const projectDir = path.join(tempDir, "project");
		fs.mkdirSync(path.join(projectDir, ".omap"), { recursive: true });
		fs.mkdirSync(path.join(tempHomeDir, ".omap", "agent"), { recursive: true });
		fs.writeFileSync(path.join(tempHomeDir, ".omap", "agent", "SYSTEM.md"), "User SYSTEM prompt");
		fs.writeFileSync(path.join(projectDir, ".omap", "SYSTEM.md"), "Project SYSTEM prompt");
