 ...
 * @oh-my-applepie/pi-natives (native Rust addon via glob). We cannot call it here.
 *
 * Instead these tests validate the structural contract that listClaudePluginRoots
 * depends on:
 *   1. OMP registry lives at path.join(home, ".omap", "plugins", "installed_plugins.json")
 *      (matches getConfigDirName() == ".omap")
...
// Kept here to avoid importing helpers.ts (which pulls in @oh-my-applepie/pi-natives).
...
// Matches getConfigDirName() — single source of truth is in @oh-my-applepie/pi-utils,
// but we know the value is ".omap" and hardcoding it here keeps tests free of
// native-addon transitive imports.
const OMP_CONFIG_DIR = ".omap";
...
describe("OMP registry path contract", () => {
	it("OMP registry lives at home/.omp/plugins/installed_plugins.json", () => {
		...
		const expected = path.join(tmpHome, ".omap", "plugins", "installed_plugins.json");
	...
	it("OMP config dir name is .omp", () => {
		...
		expect(OMP_CONFIG_DIR).toBe(".omap");
	});
});
