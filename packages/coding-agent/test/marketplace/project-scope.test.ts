 ...
 *
 * Note: helpers.ts imports @oh-my-applepie/pi-natives (Rust addon via glob).
...
import type { InstalledPluginEntry } from "@oh-my-applepie/pi-coding-agent/extensibility/plugins/marketplace";
import {
	addInstalledPlugin,
	buildPluginId,
	readInstalledPluginsRegistry,
	writeInstalledPluginsRegistry,
} from "@oh-my-applepie/pi-coding-agent/extensibility/plugins/marketplace";
import {
...
describe("resolveActiveProjectRegistryPath", () => {
	...
	it("walk-up finds nearest .omp/ directory", async () => {
		...
		fs.mkdirSync(path.join(tmpDir, ".omap"), { recursive: true });
		const cwd = path.join(tmpDir, "sub", "nested");
		fs.mkdirSync(cwd, { recursive: true });

		const result = await resolveActiveProjectRegistryPath(cwd);

		expect(result).toBe(path.join(tmpDir, ".omap", "plugins", "installed_plugins.json"));
	});
	...
	it("walk-up stops at the nearest .omp/ — does not skip to a more distant one", async () => {
		...
		fs.mkdirSync(path.join(tmpDir, ".omap"), { recursive: true });
		fs.mkdirSync(path.join(tmpDir, "sub", ".omap"), { recursive: true });
		const cwd = path.join(tmpDir, "sub", "nested");
		fs.mkdirSync(cwd, { recursive: true });

		const result = await resolveActiveProjectRegistryPath(cwd);

		expect(result).toBe(path.join(tmpDir, "sub", ".omap", "plugins", "installed_plugins.json"));
	});
	...
	it("does not treat ~/.git as a project root (pass-2 home-dir guard)", async () => {
		...
		try {
			...
			const homeOmpPath = path.join(homeDir, ".omap", "plugins", "installed_plugins.json");
...
describe("listClaudePluginRoots — project shadows user", () => {
	...
	beforeEach(() => {
		...
		fs.mkdirSync(path.join(tmpProject, ".omap", "plugins"), { recursive: true });

		userRegPath = path.join(tmpHome, ".omap", "plugins", "installed_plugins.json");
		fs.mkdirSync(path.dirname(userRegPath), { recursive: true });

		projectRegPath = path.join(tmpProject, ".omap", "plugins", "installed_plugins.json");
	});
