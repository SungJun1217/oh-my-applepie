...
A persistent per-install UUID that identifies a single oh-my-applepie installation across sessions. Used as a stable correlation key for server-side dedup of telemetry-style pushes (currently the auto-QA grievance flush from `report_tool_issue`).

## API

Exported from `@oh-my-applepie/pi-utils` (`packages/utils/src/dirs.ts`):
