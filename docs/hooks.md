...
```ts
import type { HookAPI } from "@oh-my-applepie/pi-coding-agent/extensibility/hooks";
...
`src/extensibility/hooks/index.ts` and the package subpath `@oh-my-applepie/pi-coding-agent/extensibility/hooks` export:
...
The package root (`@oh-my-applepie/pi-coding-agent`) does not re-export `HookAPI`; import legacy hook types from the hooks subpath.
