...
This document defines the JS/TS contract between `@oh-my-applepie/pi-natives` callers and the loaded N-API addon.
...
Consumers in `packages/coding-agent` and `packages/tui` import directly from `@oh-my-applepie/pi-natives`.
