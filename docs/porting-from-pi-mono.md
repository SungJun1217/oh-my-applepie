...
- Examples (adjust to match the actual packages you are porting):
  - `@mariozechner/pi-coding-agent` → `@oh-my-applepie/pi-coding-agent`
  - `@mariozechner/pi-agent-core` → `@oh-my-applepie/pi-agent-core`
  - `@mariozechner/pi-tui` → `@oh-my-applepie/pi-tui`
  - `@mariozechner/pi-ai` → `@oh-my-applepie/pi-ai`
...
- In `packages/coding-agent`, use `logger` from `@oh-my-applepie/pi-utils` for internal/runtime logging; CLI command files may use `console.*` for intentional user-facing output.
...
- Prefer existing helpers and utilities over new ad-hoc code.
  ...
  - TypeScript packages generally use source files directly; `@oh-my-applepie/pi-natives` exports generated native bindings from `packages/natives/native`.
...
| Upstream                                           | Our Fork                                                  | Reason                                        |
| -------------------------------------------------- | --------------------------------------------------------- | --------------------------------------------- |
| `clipboard.ts` + `clipboard-image.ts` (tool files) | `src/utils/clipboard.ts` backed by `@oh-my-applepie/pi-natives` | Native implementation with a small TS wrapper |
...
- `clipboard-image.ts` — image clipboard support is exposed through `src/utils/clipboard.ts` backed by `@oh-my-applepie/pi-natives`
