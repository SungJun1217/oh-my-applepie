...
`@oh-my-applepie/pi-natives` is now a two-layer package around a loader:
...
There is no current `packages/natives/src` TypeScript wrapper layer. Consumers import functions/classes/enums directly from `@oh-my-applepie/pi-natives`; the type contract is the generated `native/index.d.ts` plus enum exports appended by `scripts/gen-enums.ts`.
...
1. Consumer imports from `@oh-my-applepie/pi-natives`.
