...
This document explains how token/tool streaming is normalized in `@oh-my-applepie/pi-ai`, then propagated through `@oh-my-applepie/pi-agent-core` and `coding-agent` session events.
...
## Unified stream contract in `@oh-my-applepie/pi-ai`
