...
Most runtime lookups use `$env` from `@oh-my-applepie/pi-utils` (`packages/utils/src/env.ts`).
...
These are consumed via `@oh-my-applepie/pi-utils/dirs` and affect where coding-agent stores data.
