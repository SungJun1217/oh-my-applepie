...
- **Worker scripts**: spawn workers with the dev/compile-safe hybrid pattern. `with { type: "file" }` only copies the entry as a raw asset and does **not** bundle its imports — workers crashed silently in compiled binaries on every prior incarnation of that pattern (issues #1011, #1027). Use this shape instead:
  ```ts
  import { isCompiledBinary } from "@oh-my-applepie/pi-utils";
...
| Binary lookup   | `$which("git")` from `@oh-my-applepie/pi-utils` | `spawnSync(["which", "git"])`   |
...
- `if (await file.exists()) { await file.json() }` → two syscalls plus race. Use try-catch with `isEnoent`:
  ```typescript
  import { isEnoent } from "@oh-my-applepie/pi-utils";
  try {
...
**Rules:**
- **Tabs → spaces** via `replaceTabs()` (from `@oh-my-applepie/pi-tui` or `../tools/render-utils`).
...
**Attribution:**
- Internal (from issues): `Fixed foo bar ([#123](https://github.com/can1357/oh-my-applepie/issues/123))`.
- External contributions: `Added feature X ([#456](https://github.com/can1357/oh-my-applepie/pull/456) by [@username](https://github.com/username))`.
